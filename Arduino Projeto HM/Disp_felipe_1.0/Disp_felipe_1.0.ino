...
#define boia     A4                                     //Entrada do sensor boia
#define volt     A5
...
unsigned long long int tempoAuto = 0;
unsigned long long int tempoMe = 20000;
unsigned long long int tempoMe2 = 0;
...
int lerBut(int but)
{
  ...
  if (!digitalRead(but))   t_but   = 0x01;         //Botão Up pressionado? Seta flag
  if (digitalRead(but) && t_but == 0x01)                   //Botão Up solto e flag setada?
  { //Sim...
    t_but = 0x00;                                   //Limpa flag
...
void sensorLDR()                                        //Sensor LDR
{
  ...
  if (valorLDR < 1000) {
    disp.setCursor(9, 1);                                //Posiciona cursor na coluna 2, linha 1
    disp.print("  ");                           //Imprime mensagem
  }
  if (valorLDR < 100) {
    disp.setCursor(8, 1);                                //Posiciona cursor na coluna 2, linha 1
    disp.print(" ");
  }
  if (valorLDR < 10) {
    disp.setCursor(7, 1);                                //Posiciona cursor na coluna 2, linha 1
    disp.print(" ");
  }
...
void Voltimetro()
{
  disp.setCursor(1, 0);                                //Posiciona cursor na coluna 1, linha 1
  ...
  //if (Vi < 10) {
    disp.setCursor(10, 1);                                //Posiciona cursor na coluna 2, linha 1
    disp.print(" V");
  //}
...
{
  /*disp.setCursor(15, 0);                                //Posiciona cursor na coluna 1, linha 1
    disp.write(12);*/
...
void Barra(int PWM)
{
  ...
  for (int i = 10; i > PWM ; i--)                       //Laco comeca em 10 e vai ate o numero diminuindo
  {
    ...
    else {                                                //Se eh um dos numero entre 0 e 10 mostra a barra vazia
      disp.setCursor(i + 2, 1);
      disp.write(2);
    }
  ...
  for (int i = 1; i <= PWM; i++)                              //Laco que percore ate o numero (vai enchendo a barra)
  {
